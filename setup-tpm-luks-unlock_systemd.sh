...
# TPM2 LUKS Auto-unlock Setup Script with systemd-cryptenroll
# This script sets up automatic LUKS decryption using TPM2 and systemd-cryptenroll
...
check_systemd_version() {
    ...
    if [[ "$systemd_version" -lt 248 ]]; then
        print_error "systemd version $systemd_version is too old. Version 248 or newer is required for TPM2 support."
        return 1
    fi
...
show_luks_slots() {
    ...
    cryptsetup luksDump "$LUKS_DEVICE" | grep -E "^Key Slot|^\s+Key material offset|^  [0-9]+: luks2|Priority:" | head -20
}
...
check_tpm2_enrollment() {
    ...
    # Check if any TPM2 tokens exist
    if cryptsetup luksDump "$device" 2>/dev/null | grep -q "tpm2"; then
        print_warning "TPM2 enrollment already exists on $device"
        
        # Try to get more details
        if command_exists systemd-cryptenroll; then
            print_info "Current enrollments:"
...
# Function to bind LUKS to TPM2 using systemd-cryptenroll
bind_tpm2() {
...
manage_key_slots() {
    ...
    if check_password_exists "$NEW_PASSWORD"; then
        ...
        for i in 1 2 3 4 5 6 7 0; do  # Try slots 1-7 first, then 0
            if ! cryptsetup luksDump "$LUKS_DEVICE" | grep -qE "(Key Slot $i: ENABLED|^  $i: luks2)"; then
                new_slot=$i
                break
            fi
        done
    ...
    print_warning "The original installation password (ubuntuKey) is still active"
    ...
    if [[ "$response" =~ ^[Yy]$ ]]; then
        ...
        if [[ -n "$ubuntu_key_slot" ]]; then
            # Double-check it's not a TPM slot or our new passwords
            # Check if this slot is a regular password slot (not TPM2)
            local is_tpm_slot=false
            if cryptsetup luksDump "$LUKS_DEVICE" 2>/dev/null | grep -A5 "^  $ubuntu_key_slot:" | grep -q "tpm2"; then
                is_tpm_slot=true
            fi
            
            if [[ "$is_tpm_slot" == "false" ]] && [[ "$ubuntu_key_slot" != "$new_slot" ]] && [[ "$ubuntu_key_slot" != "$recovery_slot" ]]; then
                ...
                if [[ "$is_tpm_slot" == "true" ]]; then
                    print_warning "The identified slot is used by TPM2. Not removing."
                else
            ...
            print_info "Current non-TPM2, non-new slots: ${slots_to_check[*]}"
            print_info ""
            print_info "Manual identification method:"
...
check_setup_state() {
    ...
    if check_systemd_version >/dev/null 2>&1; then
        state_summary+="  ✓ systemd version supports TPM2\n"
    else
        state_summary+="  ✗ systemd version too old for TPM2\n"
        is_fully_configured=false
    fi
    ...
    local missing_packages
    missing_packages=0
    for pkg in systemd tpm2-tools cryptsetup; do
        if ! dpkg -l | grep -q "^ii  $pkg "; then
            ((missing_packages++))
        fi
    done
    ...
    # Check systemd-cryptenroll command
    if command_exists systemd-cryptenroll; then
        state_summary+="  ✓ systemd-cryptenroll available\n"
    else
        state_summary+="  ✗ systemd-cryptenroll not available\n"
        is_fully_configured=false
    fi
    
    # Check TPM2 enrollment
    if [[ -n "${LUKS_DEVICE:-}" ]]; then
        if check_tpm2_enrollment "$LUKS_DEVICE" >/dev/null 2>&1; then
            state_summary+="  ✓ TPM2 enrollment exists\n"
        else
            state_summary+="  ✗ TPM2 enrollment not configured\n"
            is_fully_configured=false
        fi
    fi
...
verify_setup() {
    ...
    # Check TPM2 enrollment
    if ! check_tpm2_enrollment "$LUKS_DEVICE"; then
        print_error "TPM2 enrollment not found"
        return 1
    fi
...
show_setup_plan() {
    echo
    echo "TPM2 LUKS Auto-unlock Setup Plan (systemd-cryptenroll):"
    echo "======================================================="
...
# Main execution
main() {
    print_info "Starting TPM2 LUKS Auto-unlock Setup (systemd-cryptenroll)"
    echo "=========================================================="
    ...
    # Step 2: Check systemd version
    if ! check_systemd_version; then
        exit 1
    fi
    ...
    # Step 4: Install packages first (before checking TPM2 tools)
    install_packages
    
    # Step 5: Now check TPM2 with tools
