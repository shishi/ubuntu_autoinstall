...
# TPM Status and Debug Information Script for systemd-cryptenroll
...
check_systemd_cryptenroll() {
    ...
    if check_root; then
        print_info "Checking for systemd-cryptenroll TPM2 enrollments:"
        local found=0
        
        for device in /dev/sd* /dev/nvme* /dev/vd* /dev/mapper/*; do
            if [[ -b "$device" ]] && cryptsetup isLuks "$device" 2>/dev/null; then
                # Check if device has TPM2 tokens in metadata
                if cryptsetup luksDump "$device" 2>/dev/null | grep -q "tpm2"; then
                    print_success "Found TPM2 enrollment on $device"
                    
                    # Show enrollment details
                    print_info "Enrollment details:"
                    if systemd-cryptenroll "$device" --tpm2-device=list 2>/dev/null; then
                        :
                    else
                        # Alternative method to show TPM2 tokens
                        cryptsetup luksDump "$device" 2>/dev/null | grep -A10 "Tokens:" | grep -E "(tpm2|Keyslot:)" | sed 's/^/    /'
                    fi
                    found=1
                    echo
                fi
        ...
        if [[ $found -eq 0 ]]; then
            print_info "No systemd-cryptenroll TPM2 enrollments found"
        fi
    fi
}
...
show_luks_info() {
    ...
    for device in /dev/sd* /dev/nvme* /dev/vd* /dev/mapper/*; do
        if [[ -b "$device" ]] && cryptsetup isLuks "$device" 2>/dev/null; then
            ...
            local tpm2_tokens
            tpm2_tokens=$(cryptsetup luksDump "$device" 2>/dev/null | grep -c "tpm2" || echo 0)
            if [[ $tpm2_tokens -gt 0 ]]; then
                echo "  TPM2 tokens found: $tpm2_tokens"
                
                # Show token details
                echo "  Token details:"
                cryptsetup luksDump "$device" 2>/dev/null | grep -A5 "Tokens:" | grep -E "(tpm2|Keyslot:|type:)" | sed 's/^/    /' | head -20
            fi
            echo
        fi
    done
...
show_boot_config() {
    ...
    # Check for systemd-cryptsetup services
    print_info "systemd-cryptsetup services:"
    for service in $(systemctl list-unit-files | grep systemd-cryptsetup | awk '{print $1}'); do
        local status
...
run_diagnostics() {
    ...
    # Check for TPM2 enrollments
    local tpm2_enrollment_found=0
    if check_root; then
        for device in /dev/sd* /dev/nvme* /dev/vd*; do
            if [[ -b "$device" ]] && cryptsetup isLuks "$device" 2>/dev/null; then
                if cryptsetup luksDump "$device" 2>/dev/null | grep -q "tpm2"; then
                    tpm2_enrollment_found=1
                    break
                fi
            fi
        done
    ...
    if [[ $tpm2_enrollment_found -eq 1 ]]; then
        print_success "TPM2 enrollments: FOUND"
    else
        print_warning "TPM2 enrollments: NONE FOUND (or need root)"
    fi
...
if [[ $# -gt 0 ]]; then
    case "$1" in
        ...
        diag|diagnose)
            ...
            ;;
        *)
            echo "Usage: $0 [tpm|pcr|luks|systemd|boot|diag]"
