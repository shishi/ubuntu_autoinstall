...
# TPM2 LUKS Auto-unlock Setup Script with systemd-cryptenroll (Version 6)
# Simplified logical flow - no redundant password prompts
...
remove_old_passwords() {
    ...
    # If the current password is the same as the new password, nothing to remove
    if [[ "$CURRENT_PASSWORD" == "$NEW_USER_PASSWORD" ]]; then
        print_info "Current password is the same as new password. Nothing to remove."
        return 0
    fi
    
    print_warning "The current password will be removed after setting up the new password."
    read -r -p "Continue with removing the old password? (y/N): " response
    
    if [[ ! "$response" =~ ^[Yy]$ ]]; then
        print_info "Keeping the old password"
        return 0
    fi
    ...
    if [[ -n "$current_slot" ]]; then
        print_info "Removing old password from slot $current_slot..."
        if printf '%s' "$NEW_USER_PASSWORD" | cryptsetup luksKillSlot "$LUKS_DEVICE" "$current_slot"; then
            print_success "Old password removed from slot $current_slot"
        else
            print_warning "Failed to remove old password"
        fi
    else
        print_info "Could not identify which slot contains the old password"
    fi
}
...
# Main function
main() {
    print_info "TPM2 LUKS Auto-unlock Setup (systemd-cryptenroll) - Version 6"
    echo "======================================================================"
    echo
    ...
    echo "  6. Remove the old password (if different from new)"
    echo
    ...
    # Step 6: Enroll everything in correct order
