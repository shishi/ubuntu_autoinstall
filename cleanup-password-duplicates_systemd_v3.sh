...
# Password Duplicate Cleanup Script for systemd-cryptenroll (Version 3)
# Fixed to handle single password case correctly
...
cleanup_password_duplicates() {
    ...
    if [[ ${#matching_slots[@]} -eq 0 ]]; then
        ...
        # Show slot analysis even for single match
    ...
    remove_duplicate_slots "$device" "$keep_slot" "$check_password" "${remove_slots[@]}"
