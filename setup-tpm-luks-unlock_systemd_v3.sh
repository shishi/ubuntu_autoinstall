...
# TPM2 LUKS Auto-unlock Setup Script with systemd-cryptenroll (Version 3)
...
get_installation_password() {
    ...
    # First check if we need to remove any old passwords
    local has_old_passwords=false
    
    # Count how many password slots exist (excluding TPM2 and known passwords)
    local password_slots=0
    for i in {0..7}; do
        if cryptsetup luksDump "$LUKS_DEVICE" 2>/dev/null | grep -E "^Key Slot $i: ENABLED|^  $i: luks2" | grep -q .; then
            # Check if it's not TPM2
            if ! cryptsetup luksDump "$LUKS_DEVICE" 2>/dev/null | grep -A5 "^  $i:" | grep -q "tpm2"; then
                # Check if it's not our known passwords
                ...
                # Check against new password
                if [[ -n "$NEW_USER_PASSWORD" ]] && printf '%s' "$NEW_USER_PASSWORD" | cryptsetup open --test-passphrase "$LUKS_DEVICE" --key-slot "$i" 2>/dev/null; then
                    is_known=true
                fi
                # Check against current password
                if [[ -n "$CURRENT_PASSWORD" ]] && printf '%s' "$CURRENT_PASSWORD" | cryptsetup open --test-passphrase "$LUKS_DEVICE" --key-slot "$i" 2>/dev/null; then
                    # Only count as unknown if it's different from new password
                    if [[ "$CURRENT_PASSWORD" != "$NEW_USER_PASSWORD" ]]; then
                        has_old_passwords=true
                    else
                        is_known=true
                    fi
                fi
    ...
    if [[ "$has_old_passwords" == "false" ]]; then
        print_success "No old passwords found to remove"
        NEEDS_PASSWORD_REMOVAL=false
        return 0
    fi
    
    print_info "Found $password_slots unknown password slot(s)"
    ...
    while [[ $attempts -lt 3 ]]; do
        ...
        if printf '%s' "$INSTALL_PASSWORD" | cryptsetup open --test-passphrase "$LUKS_DEVICE" 2>/dev/null; then
            ...
            if [[ $attempts -lt 3 ]]; then
                ...
                print_error "Failed to verify installation password after 3 attempts"
                print_warning "Skipping installation password removal"
                INSTALL_PASSWORD=""
                return 1
            fi
        fi
...
# Main function
main() {
    print_info "TPM2 LUKS Auto-unlock Setup (systemd-cryptenroll) - Idempotent Version"
    echo "======================================================================"
    echo
