...
# TPM2 LUKS Auto-unlock Setup Script with systemd-cryptenroll (Version 4)
# More robust version with better error handling and debugging
...
# Function to analyze password slots more safely
analyze_password_slots() {
    print_info "Analyzing LUKS password slots..."
    
    local total_slots=0
    local password_slots=0
    local known_slots=0
    local unknown_slots=0
    
    # Get LUKS version
    local luks_version
    luks_version=$(cryptsetup luksDump "$LUKS_DEVICE" 2>/dev/null | grep "^Version:" | awk '{print $2}')
    print_debug "LUKS version: $luks_version"
    
    # Count slots differently based on LUKS version
    if [[ "$luks_version" == "2" ]]; then
        ...
        while IFS= read -r line; do
            if [[ "$line" =~ ^[[:space:]]+([0-9]+):[[:space:]]*luks2 ]]; then
                ...
                ((total_slots++))
                print_debug "Found slot: $slot"
                
                # Check if it's a TPM2 slot
                if cryptsetup luksDump "$LUKS_DEVICE" 2>/dev/null | awk "/^  $slot:/{flag=1} flag && /^  [0-9]+:/{flag=0} flag" | grep -q "tpm2"; then
                    print_debug "Slot $slot is TPM2"
                    continue
                fi
                
                ((password_slots++))
                
                # Check if it's a known password
                local is_known=false
                
                # Test against recovery key
                if [[ -n "$RECOVERY_KEY" ]] && printf '%s' "$RECOVERY_KEY" | cryptsetup open --test-passphrase "$LUKS_DEVICE" --key-slot "$slot" 2>/dev/null; then
                    print_debug "Slot $slot matches recovery key"
                    is_known=true
                    ((known_slots++))
                fi
                
                # Test against new password
                if [[ "$is_known" == "false" && -n "$NEW_USER_PASSWORD" ]] && printf '%s' "$NEW_USER_PASSWORD" | cryptsetup open --test-passphrase "$LUKS_DEVICE" --key-slot "$slot" 2>/dev/null; then
                    print_debug "Slot $slot matches new user password"
                    is_known=true
                    ((known_slots++))
                fi
                
                # Test against current password
                if [[ "$is_known" == "false" && -n "$CURRENT_PASSWORD" ]] && printf '%s' "$CURRENT_PASSWORD" | cryptsetup open --test-passphrase "$LUKS_DEVICE" --key-slot "$slot" 2>/dev/null; then
                    print_debug "Slot $slot matches current password"
                    if [[ "$CURRENT_PASSWORD" != "$NEW_USER_PASSWORD" ]]; then
                        # This is an old password that needs removal
                        is_known=true
                        ((known_slots++))
                    fi
                fi
                ...
                if [[ "$is_known" == "false" ]]; then
                    print_debug "Slot $slot is unknown"
                    ((unknown_slots++))
                fi
            fi
        done < <(cryptsetup luksDump "$LUKS_DEVICE" 2>/dev/null)
    else
        # LUKS1 format
        for i in {0..7}; do
            if cryptsetup luksDump "$LUKS_DEVICE" 2>/dev/null | grep -q "Key Slot $i: ENABLED"; then
                ((total_slots++))
                ((password_slots++))
                
                # Similar checks for LUKS1
                local is_known=false
                
                if [[ -n "$RECOVERY_KEY" ]] && printf '%s' "$RECOVERY_KEY" | cryptsetup open --test-passphrase "$LUKS_DEVICE" --key-slot "$i" 2>/dev/null; then
                    is_known=true
                    ((known_slots++))
                elif [[ -n "$NEW_USER_PASSWORD" ]] && printf '%s' "$NEW_USER_PASSWORD" | cryptsetup open --test-passphrase "$LUKS_DEVICE" --key-slot "$i" 2>/dev/null; then
                    is_known=true
                    ((known_slots++))
                elif [[ -n "$CURRENT_PASSWORD" ]] && printf '%s' "$CURRENT_PASSWORD" | cryptsetup open --test-passphrase "$LUKS_DEVICE" --key-slot "$i" 2>/dev/null; then
                    if [[ "$CURRENT_PASSWORD" != "$NEW_USER_PASSWORD" ]]; then
                        is_known=true
                        ((known_slots++))
                    fi
    ...
    print_info "Slot analysis complete:"
    print_info "  Total slots: $total_slots"
    print_info "  Password slots: $password_slots"
    print_info "  Known passwords: $known_slots"
    print_info "  Unknown passwords: $unknown_slots"
    
    return $unknown_slots
}

# Function to get installation password
get_installation_password() {
    ...
    if [[ $unknown_count -eq 0 ]]; then
        print_success "No unknown passwords found to remove"
        NEEDS_PASSWORD_REMOVAL=false
        INSTALL_PASSWORD=""
        return 0
    fi
    
    print_info "Found $unknown_count unknown password slot(s)"
    print_info "If you know the installation password (e.g., ubuntuKey), please enter it"
    print_info "Otherwise, press Enter to skip"
    
    read -r -s -p "Enter installation password (or press Enter to skip): " INSTALL_PASSWORD
    echo
...
# Main function
main() {
    print_info "TPM2 LUKS Auto-unlock Setup (systemd-cryptenroll) - Version 4"
    echo "======================================================================"
    echo
