...
# Password Duplicate Cleanup Script for systemd-cryptenroll environments
...
get_luks_slots_detailed() {
    ...
    if [[ "$luks_version" == "2" ]]; then
        # LUKS2 format - need to check tokens for systemd-managed slots
        local -a systemd_slots=()
        local in_tokens=false
        local in_keyslots=false
        local current_keyslot=""
        
        # First, get systemd-managed slots from Tokens section
        while IFS= read -r line; do
            if [[ "$line" =~ ^Tokens: ]]; then
                ...
                # Look for systemd-managed tokens
                if [[ "$line" =~ [[:space:]]+type:[[:space:]]+(systemd-tpm2|systemd-fido2|systemd-pkcs11) ]]; then
                    # Next Keyslot line will have the slot number
                    local next_line
                    if read -r next_line && [[ "$next_line" =~ [[:space:]]+Keyslot:[[:space:]]+([0-9]+) ]]; then
                        systemd_slots+=("${BASH_REMATCH[1]}")
                    fi
                fi
                ...
                if [[ " ${systemd_slots[*]} " =~ " ${current_keyslot} " ]]; then
                    slot_type="systemd-managed"
                fi
                
                slot_info+=("$current_keyslot:$slot_type")
            fi
        done < <(cryptsetup luksDump "$device" 2>/dev/null)
    else
        # LUKS1 format - all slots are password slots
        for i in {0..7}; do
            if cryptsetup luksDump "$device" 2>/dev/null | grep -q "Key Slot $i: ENABLED"; then
                slot_info+=("$i:password")
            fi
        done
    fi
...
test_password_on_slots() {
    ...
    for info in "${slot_info[@]}"; do
        ...
        # Only test password slots (not systemd-managed slots)
...
display_slot_info() {
    ...
    # Show tokens if LUKS2
    local luks_version
    luks_version=$(cryptsetup luksDump "$device" 2>/dev/null | grep "^Version:" | awk '{print $2}')
    
    if [[ "$luks_version" == "2" ]]; then
        echo
        print_info "Tokens (systemd-managed slots):"
        cryptsetup luksDump "$device" 2>/dev/null | sed -n '/^Tokens:/,/^[A-Z]/p' | grep -E "(^Tokens:|type:|Keyslot:)" | head -20 || print_info "  No tokens found"
    fi
    ...
    if command_exists systemd-cryptenroll; then
        ...
        systemd-cryptenroll "$device" --tpm2-device=list 2>/dev/null || print_info "  No systemd-cryptenroll enrollments"
    fi
...
SAFETY FEATURES:
    ...
    - Only removes password slots (preserves systemd-managed slots)
    - Supports dry-run mode

NOTE:
    This script only checks regular password slots.
    systemd-managed slots (TPM2, FIDO2, etc.) are ignored.
